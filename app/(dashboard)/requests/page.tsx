...
// NOTE: This page is temporarily disabled and not in use
// TODO: Re-enable when document request functionality is needed
...
import { useFetchClientRequests } from '@/hooks/useDocumentRequests'
...
export default function ClientDocumentRequestsPage() {
  ...
  const { requests, loading, error, fetchRequests } = useFetchClientRequests()
  ...
  // Fetch document requests when clientId is available
  useEffect(() => {
    if (clientId) {
      fetchRequests(clientId, 10)
    }
  ...
  const handleUpload = async (requestId: string, file: File, notes: string) => {
    ...
    try {
      ...
      if (clientId) {
        await fetchRequests(clientId, 5)
      }
    } catch (err: any) {
...
//               <DocumentRequestCard key={request.documentRequestId} request={request} onUpload={handleUpload} />
//             ))
//           ) : (
//             <Card>
...
export default function ClientDocumentRequestsPage() {
  return (
    <div className="flex items-center justify-center min-h-screen">
      <div className="text-center">
        <h1 className="text-3xl font-bold mb-4">Page Disabled</h1>
        <p className="text-muted-foreground">This page is currently disabled and not in use.</p>
      </div>
    </div>
  )
}
